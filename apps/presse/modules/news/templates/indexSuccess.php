...
<div class="box">
  ...
  <div class="content">
    ...
    <? foreach ($pager->getResults() as $actualite): ?>
      <li style="line-height: 25px;"><?=format_date($actualite->getPublishOn(), 'dd/MM')?> - <?=link_to($actualite->getTitle(), 'news/view?slug='.$actualite->getSlug())?></li>
    <? endforeach; ?>
    </ul>

    <div class="pager pager-bottom" style="border-top: solid 1px #aaa; text-align: center;">
      <span class="page"><a href="<?=url_for('news/index?page='.$pager->getFirstPage())?>"><<</a></span> 
      ...
      <span class="page"><a href="<?=url_for('news/index?page='.$pager->getLastPage())?>">>></a></span>
    </div>
  </div>
</div>
