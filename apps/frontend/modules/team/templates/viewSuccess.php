    ...
    <? if (isset($idTeam)): ?>
        ...
        if ($alreadyInTournament == true) {
            foreach($tournaments as $tournament): ?>
                <?=__('L equipe est inscrite au tournoi ')?>;
                <?=link_to($tournament->getTournament()->getName(), 'tournament/view?slug='.$tournament->getTournament()->getSlug())?>
            <? endforeach;
        }
        else if ($alreadyInTournament == false) 
        { ?>
            <?=__('L equipe n est pas inscrite a un tournoi')?>
		<?
        };
        endif;
                ...
                if (!isset($idTeam)):
?>
<?=__('Vous n appartenez a aucune equipe pour le moment.')?><br/>
<?=link_to(__('Creer une nouvelle equipe'), 'team/new'); ?>
<? endif; ?>
</div>
