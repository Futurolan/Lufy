...
<form class="form-horizontal" action="<?php echo url_for('game_type/'.($form->getObject()->isNew() ? 'form' : 'form').(!$form->getObject()->isNew() ? '?id_game_type='.$form->getObject()->getIdGameType() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
...
<ul class="breadcrumb subbreadcrumb">
  ...
  <?php if (!$form->getObject()->isNew()): ?>
    <li><?php echo link_to('<i class="icon icon-remove-sign"></i> '.__('Delete'), 'game_type/delete?id_game_type='.$form->getObject()->getIdGameType(), array('method' => 'delete', 'confirm' => __('Are you sure ?'))) ?> <span class="divider">|</span></li>
  <?php endif; ?>
