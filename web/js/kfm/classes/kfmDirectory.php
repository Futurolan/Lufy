...
class kfmDirectory extends kfmObject{
	...
	function createSubdir($name){
		...
		if(!$this->checkAddr($physical_address) ){
			$this->error(kfm_lang('illegalDirectoryName',$short_version). ' pb with $physical_address: '.$physical_address);
			return false;
		}
		*/
		if(!$this->checkAddr( !$this->checkName($name))){
			$this->error(kfm_lang('illegalDirectoryName',$short_version). 'pb with name: '.$name);
			return false;
		}		
