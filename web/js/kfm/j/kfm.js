...
kfm.alert = function(txt) {
	$j('<div title="Alert"></div>').append(txt).dialog( {
		modal : true
	});
};
...
kfm.build = function() {
				...
				kfm.lang[j] = (function(str) {
					return function() {
						...
						for (i = 1; i < arguments.length + 1; ++i)
							tmp = tmp.replace("%" + i, arguments[i - 1]);
						return tmp;
					};
				})(kfm.lang[j]);
			}
	...
	if (kfm_vars.use_templates) {
		...
		if (!documents_body)
			alert('no #documents_body on page - please fix your template');
		var wrapper = document.getElementById('kfm_search_wrapper');
		if (wrapper)
			wrapper.appendChild(kfm_searchBoxFile());
		var wrapper = document.getElementById('kfm_upload_wrapper');
		if (wrapper)
			kfm.addEl(wrapper, kfm_createFileUploadPanel(true));
	} else {
		{
			var left_column = kfm_createPanelWrapper('kfm_left_column');
			kfm_resizeHandler_addMaxHeight('kfm_left_column');
			$each( [ 'kfm_directories_panel', 'kfm_widgets_panel',
					'kfm_search_panel', 'kfm_directory_properties_panel' ],
					function(panel) {
						if (!kfm_inArray(panel, kfm_hidden_panels))
							kfm_addPanel(left_column, panel);
					});
			...
			kfm_addContextMenu(left_column, function(e) {
				...
				for ( var i = 0; i < ps.length; ++i) {
					...
					var panel_name = ps[i];
					if (!p.visible && !kfm_inArray(ps[i], kfm_hidden_panels))
						links.push( {
							title : 'Show panel ' + p.panel_title,
							doFunction : function() {
								kfm_addPanel(document
										.getElementById('kfm_left_column'),
										panel_name);
							}
						});
				}
		...
		}
		{
			...
			if (kfm_vars.show_admin_link)
				hhtml += '<a href="admin/" id="admin_panel_link">Admin panel</a>';
	...
	}
	{
		...
		$j.event.add(documents_body, 'mousedown', function(e) {
			...
			if (this.contentMode == 'file_icons' && this.fileids.length)
				window.dragSelectionTrigger = setTimeout(function() {
					kfm_selection_dragStart()
				}, 200);
			$j.event.add(documents_body, 'mouseup', kfm_selection_dragFinish);
		});
		kfm_addContextMenu(documents_body, function(e) {
			var links = [], i;
			if (kfm_vars.permissions.file.mk)
			...
			if (selectedFiles.length > 1 && kfm_vars.permissions.file.ed)
				context_categories['edit'].add( {
					name : 'files_rename',
			...
			if (selectedFiles.length > 1 && kfm_vars.permissions.file.mk)
			...
			if (selectedFiles.length != document
					.getElementById('documents_body').fileids.length)
	...
	$j.event.add(window, 'resize', function() {
		kfm_resizeHandler();
	});
	kfm_contextmenuinit();
...
kfm.getContainer = function(p, c) {
	for ( var i = 0; i < c.length; ++i) {
		...
		if (x < p.x && y < p.y && x + a.offsetWidth > p.x
				&& y + a.offsetHeight > p.y)
			return a;
	}
};
...
kfm.keyup = function(e) {
	...
	case 13: {
		if (!selectedFiles.length || window.inPrompt || cm != 'file_icons')
			return;
		if (selectedFiles.length > 1) {
			var files = $j.extend( [], selectedFiles);
			var openingHook = kfm_getDefaultOpener(files);
			if (openingHook)
				openingHook.doFunction(files);
		} else {
			var id = selectedFiles[0];
			var openingHook = kfm_getDefaultOpener( [ id ]);
			if (openingHook)
				openingHook.doFunction( [ id ]);
		}
		break;
	}
	case 27: {
		if (cm == 'lightbox')
			kfm_img_stopLightbox();
		else if (!window.inPrompt)
			kfm.confirm(kfm.lang.AreYouSureYouWantToCloseKFM, function() {
				window.close()
			});
		break;
	}
	case 37: {
		if (cm == 'file_icons') {
			...
			if (window.lightbox_slideshowTimer)
				clearTimeout(window.lightbox_slideshowTimer);
	...
	case 113: {
		...
		if (!selectedFiles.length)
			return kfm
					.alert(_("Please select a file before you try to rename it"));
		if (selectedFiles.length == 1) {
			kfm_renameFile(selectedFiles[0]);
		} else
			kfm.alert(_("You can only rename one file at a time"));
		break;
	}
	case 127: {
		kfm_delete(cm);
...
function kfm_delete(cm) {
	if (!selectedFiles.length || cm != 'file_icons')
		return;
	if (selectedFiles.length > 1)
		kfm_deleteSelectedFiles();
	else
		kfm_deleteFile(selectedFiles[0]);
}
...
function kfm_prompt(txt, val, fn) {
	...
	$j('<div id="kfm-prompt" title="Prompt"></div>').append(txt).append(
			'<input id="kfm_prompt_input" value="' + val + '"/>').dialog(
			{
				modal : true,
				buttons : {
					Ok : function() {
				...
				open : function() {
					...
					$j('#kfm-prompt input').keyup(
							function(e) {
								if (e.which == 13) {
									$j('#kfm-prompt').closest('.ui-dialog')
...
function kfm_shrinkName(name, wrapper, text, size, maxsize, extension) {
	var position = step = Math.ceil(name.length / 2), postfix = '[...]'
			+ extension, prefix = size == 'offsetHeight' ? '. ' : '';
	do {
		text.innerHTML = prefix + name.substring(0, position) + postfix;
		step = Math.ceil(step / 2);
		position += (wrapper[size] > maxsize) ? -step : step;
	} while (step > 1);
	var html = '<span class="filename">'
			+ name.substring(0, position + (prefix ? 0 : -1))
			+ '</span><span style="color:red;text-decoration:none">[...]</span>';
	if (extension)
		html += '<span class="filename">' + extension + '</span>';
	text.innerHTML = html;
}
		...
		$j(jDiv)
				.slideDown(
						...
						function() {
							...
							$j(this)
									.append(
											'<iframe id="plugin_iframe_element" src="'
													+ url
...
function kfm_pluginIframeButton(code, text) {
	...
	if (!hdr)
		return;
	if (code == 'close') {
		btn = $j('<img src="themes/' + kfm_theme + '/icons/remove.png"/>')
				.click(function() {
		...
		btn = $j('<span class="kfm_plugin_iframe_button"></span>').click(
				function() {
					eval(code);
				});
	}
...
function kfm_pluginIframeMessage(message) {
	...
	setTimeOut(
			'document.getElementById("plugin_iframe_message").style.display="none";',
			3000);
}
